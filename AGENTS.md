...
This is repository provides a reusable GitHub Workflow for generating and publishing OpenAPI clients to npm. It uses the `workflow_call` trigger pattern, allowing external repositories to invoke it.
...
1. **Reusable Workflow** (`.github/workflows/generate-and-publish.yml`)
   ...
   - Publishing secured via npm Trusted Publishing (OIDC) — only repos with a matching Trusted Publisher on npmjs.com can publish
...
1. **OIDC Trusted Publishing** - Only repos with a matching Trusted Publisher configured on npmjs.com can publish
2. **Provenance attestation** - Automatically generated by npm Trusted Publishing, tying published packages to their source repo and workflow
3. **Secrets are masked** - GitHub automatically masks secret values in logs
